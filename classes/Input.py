...
class Input:
    ...
    def checkForKeyboardInput(self):
        ...
        # Movement (A/D instead of LEFT/RIGHT)
        if (pressedKeys[K_a] or pressedKeys[K_h]) and not pressedKeys[K_d]:
            self.entity.traits["goTrait"].direction = -1
        elif (pressedKeys[K_d] or pressedKeys[K_l]) and not pressedKeys[K_a]:
            self.entity.traits["goTrait"].direction = 1
        else:
            self.entity.traits['goTrait'].direction = 0

        # Jump (W instead of UP)
        isJumping = pressedKeys[K_w] or pressedKeys[K_SPACE] or pressedKeys[K_k]
        self.entity.traits['jumpTrait'].jump(isJumping)

        # Boost stays the same
        self.entity.traits['goTrait'].boost = pressedKeys[K_LSHIFT]

        # Optional: Crouch with S
        if pressedKeys[K_s]:
            if hasattr(self.entity, "crouch"):
                self.entity.crouch()
    ...
    def checkForMouseInput(self, events):
        ...
        # Right click triggers jump
        if self.isRightMouseButtonPressed(events):
            self.entity.traits['jumpTrait'].jump(True)

        # Left click still spawns coins
        if self.isLeftMouseButtonPressed(events):
            self.entity.levelObj.addCoin(
                mouseX / 32 - self.entity.camera.pos.x, mouseY / 32
            )
    ...
    def checkMouse(self, events, button):
        for e in events:
            # Trigger jump/action immediately on press
